...
def are_valid_rates(rates):
  ...
  rate_names = []
  for rate in rates:
    if any(field not in rate for field in ['name', 'amount']):
      return False
    if int(rate['amount']) <= 0 or rate['name'] in rate_names:
      return False
...
def are_valid_deliverables(deliverables):
  ...
  for deliverable in deliverables:
    if any(field not in deliverable for field in ['name', 'description', 'estimate', 'rate']):
      return False
    if float(deliverable['estimate']) <= 0 or deliverable['name'] in deliverable_names:
      return False
